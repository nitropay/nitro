 ...
 *
 *   // network
 *   CNitroPayNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.nitropay.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CNitroPayNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CNitroPayNetwork(UniValue objIn)
    {
...
// // can be under: NitroPayNetwork
// //   -- signature requirements : Key1(User)
// class CNitroPayNetworkVariable : public CGovernanceObject
// {
// private:
