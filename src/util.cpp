...
const char * const BITCOIN_CONF_FILENAME = "nitropay.conf";
const char * const BITCOIN_PID_FILENAME = "nitropayd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nitropay" is a composite category enabling all NitroPay-related debug output
            if(ptrCategory->count(string("nitropay"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NitroPayCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NitroPayCore
    // Mac: ~/Library/Application Support/NitroPayCore
    // Unix: ~/.nitropaycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NitroPayCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NitroPayCore";
#else
    // Unix
    return pathRet / ".nitropaycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nitropay.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nitropay.conf
